...
public:
	...
	bool IsUniqueMST();
	int findParent(const vector<int>& parent, int v) const;
};
...
//Solin算法求最小生成树
template <class ElemType, class WeightType>
int AdjListUndirNetwork<ElemType, WeightType>::findParent(const vector<int>& parent, int v) const {
	while (parent[v] != v) {
		v = parent[v];
	}
	return v;
...
void AdjListUndirNetwork<ElemType, WeightType>::GenerateAndPrintMSTByCycleBreaking() {
	...
	// 动态分配边指针数组
	unique_ptr<Edge<WeightType>[]> edges(new Edge<WeightType>[edgeCount]);
	...
	// 按权重升序排序
	sort(edges.get(), edges.get() + edgeCount, [](const Edge<WeightType>& a, const Edge<WeightType>& b) {
	...
	while (mstEdges.size() < vexNum - 1 && edgeIndex < edgeCount) {
		int u = edges[edgeIndex].from;
		int v = edges[edgeIndex].to;
		WeightType w = edges[edgeIndex].weight;
		...
		if (!IsConnected()) {
			...
			InsertArc(v, u, w);
			arcNum++;
			mstEdges.push_back(edges[edgeIndex]);
		}
	...
	// 打印结果
	cout << "\n=== 最小生成树结果 ===" << endl;
	if (mstEdges.size() == vexNum - 1) {
		cout << "边数: " << mstEdges.size() << "/" << vexNum - 1 << "（满足树条件）\n";
		cout << "边列表（顶点1-顶点2 : 权重）:\n";
		for (const auto& edge : mstEdges) {
